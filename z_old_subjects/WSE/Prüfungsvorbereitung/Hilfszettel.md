...
- git merge feature (Quellbranch) (ziel branch ist der der ausgewählt)
...
- git clone: lokale Kopie von entferntem repo
	- Name ist standardmäßig origin, kann durch -o <\name> geändert werden
	- meim clone werden alle branches runtergeladen und als remote tracking banch gespeichert, automatisch wird der branch ausgecheckt auf den der head im remote repo zeigt
	- wenn man sie auscheckt werden sie in local branches umgewandelt und man kann committen
...
- git push: -u (neuer branch im remote repo) -d (löscht branch im rem. repo) -f (force push)


- local branches
	- non-tracking local branches
		- "normaler" branch, einfach ein Zeiger auf einen commit
		- wie ein Zeiger auf einen Commit, jedoch wird oft die ganze devline gemeint
	- tracking local branches
		- branch mit Referenz auf anderen branch, z.B. auf einen remote tracking branch
		- nach ersten mal git push -u origin feature:feature ist der lokale branch ein local tracking branch
