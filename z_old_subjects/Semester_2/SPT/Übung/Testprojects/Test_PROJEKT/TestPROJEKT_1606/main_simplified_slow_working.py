...
try:
    ...
    DRIVE_SPEED = 110    # Normale Geschwindigkeit auf hellem Boden (mm/s)
    ...
    BACKWARD_SPEED     = -90   # Deutlich spürbares Rückwärtstempo (mm/s)
    BACKWARD_TURN_RATE = 140   # Moderate Drehrate für eine schöne Kurve (deg/s)
    BACKWARD_TIME      = 350   # Dauer der Ausweichkurve in Millisekunden
    ...
    while True:
        ...
        if left_sees_black and right_sees_black:
            # Beide Sensoren sehen schwarz (z.B. senkrechte Wand/Kreuzung)
            # Kurzes Stück zurücksetzen, um wieder im Korridor zu sein
            current_state = "Beide Schwarz"
            robot.drive(-60, 0)
            wait(250) 
            
        elif left_sees_black:
            # Linker Sensor berührt Grenze -> Scharf nach RECHTS wegdrehen
            current_state = "Kurve Rechts"
            robot.drive(TURN_SPEED, -TURN_RATE * STEERING_MULTIPLIER)
            wait(300)
            
        elif right_sees_black:
            # Rechter Sensor berührt Grenze -> Scharf nach LINKS wegdrehen
            current_state = "Kurve Links"
            robot.drive(TURN_SPEED, TURN_RATE * STEERING_MULTIPLIER)
            wait(300)
