...
CREATE TABLE Mitarbeiter( -- at least six per VO
    ...
    VorgesetztenID NUMBER(9),

    CONSTRAINT Mitarbeiter_pk PRIMARY KEY (MANr),
    CONSTRAINT VorgesetztenID_fk FOREIGN KEY (VorgesetztenID)
...
CREATE TABLE Premiumkunde(
  ...
  --Nachname VARCHAR(30),
    ...
    CONSTRAINT Premiumkunde_KNr_fk FOREIGN KEY (KNr)
...
CREATE TABLE Sitzplatz(
    ...
    CONSTRAINT Sitzplatz_pk PRIMARY KEY (PNr, SNr, VOName),
...
CREATE TABLE Ticket (
    ...
    CONSTRAINT Ticket_Sitzplatz_fk
        FOREIGN KEY (PNr, SNr, VOName)
        REFERENCES Sitzplatz(PNr, SNr, VOName)
);
