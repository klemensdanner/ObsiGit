...
WHERE MANr IN (SELECT MANr
                ...
                GROUP BY MANR
                HAVING COUNT(VONAME) = (SELECT COUNT(*)
...
SELECT VORNAME, NACHNAME, TNR, TO_CHAR(ZEITPUNKT, 'DD.MM.YYYY')
  FROM (Premiumkunde JOIN KUNDE USING(KNR)) JOIN reserviert USING (KNr)
  WHERE geburtsdatum IN ( SELECT MAX(GEBURTSDATUM) AS YOUNGEST
  FROM Premiumkunde JOIN reserviert USING (KNR) JOIN Ticket USING (TNR)
  GROUP BY VID);

  --jüngster Premiumkunde pro Aufführung
  SELECT KNR, MAX(GEBURTSDATUM) AS YOUNGEST
  FROM Premiumkunde JOIN reserviert USING (KNR) JOIN Ticket USING (TNR)
  GROUP BY VID;
...
--10. 
SELECT TO_CHAR(GEBURTSDATUM, 'DY, DD. MONTH YYYY') 
FROM Premiumkunde
